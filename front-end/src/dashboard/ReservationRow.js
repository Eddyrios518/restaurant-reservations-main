...
export default function ReservationRow({ reservation, loadDashboard }) {
    if (!reservation || reservation.status === "finished") return null; // if there is no reservation passed through or the status = finsihed, return null

    /** this handles if the user wants to cancel a reservation */ 
    function handleCancel() {
        if (  // updates reservation status if user confirms
 ...
 /** displays a single reservations for the given day -- each single reservation is mapped in 'reservationsJSX()' in 'Dashboard' to create a list of reservations */
    return (    
