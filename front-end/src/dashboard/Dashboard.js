...
function handleClick({ target }) {
  ...
  if (!date) {
    useDate = today(); // to set reservation's list date to current day
  } else {
    useDate = date;
  }

  if (target.name === "previous") {
    newDate = previous(useDate); // to set reservation's list date to the previous day
  } else if (target.name === "next") {
    newDate = next(useDate); // to set reservation's list date to be the following day
  } else {
    newDate = today();
  }
